...
//
// locationchange    Fires when the user first causes the location of a browser
//                   to change. This can be by clicking a link, using the back/
//                   forward buttons - whatever.
//
...
//
// For locationchange, it has:
//
//      browser -- same as tabload
//      isTop -- same as pageshow
//
//      IMPORTANT:
//      Note that locationchange occurs so early that the objects
//      in browser.contentWindow and browser.contentDocument will be the ones
//      from the *previous* page viewed in that browser, not the new one.
//
...
//       members most likely have already been torn down.
//
...
function G_TabbedBrowserWatcher(tabBrowser, name, opt_filterAboutBlank) {
  ...
  // Add an event listener for when the chrome window that owns us closes.
 ...
 * @returns {Object?} Reference to the browser in which the given document is
 ...
 * @returns {nsIDOMHTMLDocument} Reference to the Document with that URL or null 
 *                               if not found
 */
 ...
 * @returns {Object?} Reference to the browser in which sub resides, else null
 *          if not found
 */
 ...
 * @returns {Object?} Reference to the browser's tab element, or null
 */
...
function G_BrowserWatcher(tabbedBrowserWatcher, browser) {
  ...
  // Watch the nsIWebProgress interface on the browser for onLocationChange
  //
  // TODO: Does not unregistering this leak? I tried unregistering before in
  // onUnload, but browser.xml was throwing an error because docShell was null
  // at that point.
...
/**
 * Invoked when a window begins loading a new URL
 *
 * See nsIWebProgressListener.onLocationChange for details.
 */
G_BrowserWatcher.prototype.onLocationChange = function(webProgress,
                                                       request, location) {
  G_Debug(this, "onLocationChange for {%s}".subs(location.spec));
  if (!this.parent_) {
    // It's possible for this to fire after this.parent_ is nulled out on
    // tab close.  We just ignore the event in that case.
    G_Debug(this, "onLocationChange after tab close");
    return;
  }

  // TODO: figure out isTop
...
G_BrowserWatcher.prototype.onLoad = function(e) {
  ...
  if (!e.target) {
    // For an unknown reason, e.target is sometimes null.
...
G_BrowserWatcher.prototype.onUnload = function(e) {
  ...
  // We also get wierd spurious unloads on some framed pages. Real tab unloads
  // should not have a valid dochsell. Was seeing this during load of gmail and
  // caribou.
  ...
  if (!doc) {                // This is a closing tab
    ...
    // We can't remove the web progress listener.  See comment in
    // in constructor.
